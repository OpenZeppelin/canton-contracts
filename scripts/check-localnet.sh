#!/usr/bin/env bash
#
# LocalNet validation gate for the token package: runs the LocalNet scripts
...
command -v dpm >/dev/null 2>&1 || {
	printf 'check-localnet: dpm is not available\n' >&2
	exit 1
}
command -v java >/dev/null 2>&1 || {
	printf 'check-localnet: Java is not available\n' >&2
	exit 1
}
command -v lsof >/dev/null 2>&1 || {
	printf 'check-localnet: lsof is not available\n' >&2
	exit 1
}

java_version="$(java -version 2>&1 | sed -n '1s/.*version "\([0-9][0-9]*\).*/\1/p')"
[ -n "$java_version" ] && [ "$java_version" -ge 21 ] || {
	printf 'check-localnet: Java 21 or newer is required\n' >&2
	exit 1
}
...
LOG_DIR="${OZ_LOCALNET_LOG_DIR:-$ROOT/.cache/localnet-token}"
mkdir -p "$LOG_DIR"

printf 'check-localnet: building the token test package\n'
(cd "$ROOT" && DAML_PACKAGE="$TEST_PKG" dpm build)
[ -f "$DAR" ] || {
	printf 'check-localnet: expected DAR not found: %s\n' "$DAR" >&2
	exit 1
}
...
wait_for_port_release() {
	...
	printf 'check-localnet: Ledger API port %s remains in use after cleanup\n' "$port" >&2
	return 1
}
...
cleanup() {
	...
	if process_group_alive "$SANDBOX_PGID"; then
		printf 'check-localnet: stopping sandbox (pid %s)\n' "$SANDBOX_PID"
...
if [ "$USE_EXTERNAL_LEDGER" = 1 ]; then
	printf 'check-localnet: using external ledger at %s:%s (no sandbox started; must be static-time)\n' "$LEDGER_HOST" "$LEDGER_PORT"
else
	if lsof -nP -iTCP:"$LEDGER_PORT" -sTCP:LISTEN >/dev/null 2>&1; then
		printf 'check-localnet: Ledger API port %s is already in use\n' "$LEDGER_PORT" >&2
		exit 1
	fi
	set -m
	printf 'check-localnet: starting static-time Canton sandbox on %s:%s\n' "$LEDGER_HOST" "$LEDGER_PORT"
	...
	[ "$ready" = 1 ] || {
		printf 'check-localnet: sandbox did not become ready; see %s\n' "$LOG_DIR/sandbox.log" >&2
		exit 1
	}
fi

SCRIPTS=(
	localnetMintAndQuery
	localnetTransferLifecycle
	localnetAllowanceLifecycle
	localnetMintAndBurn
)

fail=0
for s in "${SCRIPTS[@]}"; do
	name="OpenZeppelin.TokenCIP112V1LocalNetTest:$s"
	log="$LOG_DIR/$s.log"
	if (cd "$ROOT/$TEST_PKG" && dpm script --dar "$DAR" --script-name "$name" \
		...
		printf 'check-localnet: PASS %s\n' "$s"
	else
		printf 'check-localnet: FAIL %s (see %s)\n' "$s" "$log" >&2
		fail=1
	fi
done

[ "$fail" = 0 ] || {
	printf 'check-localnet: FAILED\n' >&2
	exit 1
}
printf 'check-localnet: OK - all %d token scripts passed on LocalNet\n' "${#SCRIPTS[@]}"
